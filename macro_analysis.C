...
void analysis()
{
  ...
  TCanvas *cCanvas = new TCanvas("Histograms", "Histo");
  cCanvas->Divide(2, 2);
  ...
  cCanvas->cd(4);
  h->SetTitle("Abbondanza di Particelle");
  h->GetXaxis()->SetTitle("Indice Particella");
  ...
  cCanvas->Print("Angles.gif");

  cSubtraction->Print("Subtraction.pdf");
  cCanvas->Print("Canvas.pdf");
  file->Close();
}